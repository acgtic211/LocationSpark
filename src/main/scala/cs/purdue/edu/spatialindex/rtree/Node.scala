 ...
 *
 * This generalizes Node[V] (the nodes of the tree) and Entry[V] (the
...
sealed abstract class Node[V] extends HasGeom { self =>
   ...
   */
  def iterator: Iterator[Entry[V]] = this match {
    case Leaf(children, _) =>
      children.iterator
    case Branch(children, _) =>
   ...
   */
  def pretty: String = {
    def prettyRecur(node: Node[V], i: Int, sb: StringBuilder): Unit = {
      val pad = " " * i
      val a = node.box.area
      node match {
   ...
   */
  def searchIterator(space: Box, f: Entry[V] => Boolean): Iterator[Entry[V]] =
   ...
   */
  def nearest(pt: Point, d0: Double): Option[(Double, Entry[V])] = {
    var dist: Double = d0
    var result: Option[(Double, Entry[V])] = None
    this match {
      case Leaf(children, box) =>
  ...
  def count(space: Box): Int =
    if (!space.isFinite) 0 else {
      def recur(node: Node[V]): Int = node match {
        case Leaf(children, box) =>
          var n = 0
          var i = 0
          while (i < children.length) {
   ...
   * node.
   */
  def map[B](f: V => B): Node[B] = this match {
    case Leaf(cs, box) =>
...
object Node {
   ...
   */
  def splitLeaf[V](children: Vector[Entry[V]]): Vector[Leaf[V]] = {
   ...
   * See splitter for more information.
   */
  def splitBranch[V](children: Vector[Node[V]]): Vector[Branch[V]] = {
